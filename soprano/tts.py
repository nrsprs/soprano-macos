...
class SopranoTTS:
    def __init__(
        self, backend="auto", device="cuda", cache_size_mb=10, decoder_batch_size=1
    ):
        RECOGNIZED_DEVICES = ["cuda", "mps"]
        RECOGNIZED_BACKENDS = ["auto", "lmdeploy", "transformers"]
        assert device in RECOGNIZED_DEVICES, (
            f"unrecognized device {device}, device must be in {RECOGNIZED_DEVICES}"
        )
        if backend == "auto":
            if device == "cpu":
        ...
        assert backend in RECOGNIZED_BACKENDS, (
            f"unrecognized backend {backend}, backend must be in {RECOGNIZED_BACKENDS}"
        )
        ...
        self.decoder = SopranoDecoder(dev=device)
        if device == "cuda":
            self.decoder = self.decoder.cuda()
        elif device == "mps":
            self.decoder = self.decoder.to("mps")
        decoder_path = hf_hub_download(
            repo_id="ekwek/Soprano-80M", filename="decoder.pth"
        )
        device_map = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.decoder.load_state_dict(
            torch.load(decoder_path, map_location=device_map)
        )
    ...
    def infer(
        self, text, out_path=None, top_p=0.95, temperature=0.3, repetition_penalty=1.2
    ):
    ...
    def infer_batch(
        self, texts, out_dir=None, top_p=0.95, temperature=0.3, repetition_penalty=1.2
    ):
        ...
        for i, response in enumerate(responses):
            if response["finish_reason"] != "stop":
                print(
                    f"Warning: some sentences did not complete generation, likely due to hallucination."
                )
        ...
        for idx in range(0, len(hidden_states), self.decoder_batch_size):
            ...
            lengths = list(
                map(
                    lambda x: x.size(0),
                    hidden_states[idx : idx + self.decoder_batch_size],
                )
            )
            N = len(lengths)
            for i in range(N):
            ...
            for i in range(N):
                ...
                audio_concat[text_id][sentence_id] = audio[i].squeeze()[
                    -(lengths[i] * self.TOKEN_SIZE - self.TOKEN_SIZE) :
                ]
        ...
        if out_dir:
            ...
            for i in range(len(audio_concat)):
                wavfile.write(
                    f"{out_dir}/{i}.wav", 32000, audio_concat[i].cpu().numpy()
                )
        return audio_concat

    def infer_stream(
        self, text, chunk_size=1, top_p=0.95, temperature=0.3, repetition_penalty=1.2
    ):
        ...
        for sentence, _, _ in sentence_data:
            ...
            for token in responses:
                ...
                hidden_states_buffer = hidden_states_buffer[
                    -(2 * self.RECEPTIVE_FIELD + chunk_size) :
                ]
                if (
                    finished
                    or len(hidden_states_buffer) >= self.RECEPTIVE_FIELD + chunk_size
                ):
                    if finished or chunk_counter == chunk_size:
                        batch_hidden_states = torch.stack(hidden_states_buffer)
                        inp = (
                            batch_hidden_states.unsqueeze(0)
                            .transpose(1, 2)
                            .to("cuda" if torch.cuda.is_available() else "mps")
                            .to(torch.float32)
                        )
                        ...
                        if finished:
                            audio_chunk = audio[
                                -(
                                    (self.RECEPTIVE_FIELD + chunk_counter - 1)
                                    * self.TOKEN_SIZE
                                    - self.TOKEN_SIZE
                                ) :
                            ]
                            ...
                            audio_chunk = audio[
                                -(
                                    (self.RECEPTIVE_FIELD + chunk_size)
                                    * self.TOKEN_SIZE
                                    - self.TOKEN_SIZE
                                ) : -(
                                    self.RECEPTIVE_FIELD * self.TOKEN_SIZE
                                    - self.TOKEN_SIZE
                                )
                            ]
                        chunk_counter = 0
                        if first_chunk:
                            print(
                                f"Streaming latency: {1000 * (time.time() - start_time):.2f} ms"
                            )
                            first_chunk = False
